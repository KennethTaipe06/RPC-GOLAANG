...
// @Accept  json
// @Produce  json
// @Param city query string true "Ciudad del usuario"
// @Success 200 {string} string "Saludo de bienvenida"
// @Failure 400 {string} string "Error: la ciudad no puede estar vacía"
// @Router /hello [get]
func (g *Greeter) SayHello(args *Args, reply *string) error {
	if args.City == "" {
		return errors.New("city cannot be empty")
	}
	*reply = fmt.Sprintf("tu ciudad es: %s! RPC!", args.City)
	return nil
}
...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		// Si es una solicitud POST (cuando de la ciudad envía el formulario)
		if r.Method == "POST" {
			r.ParseForm()
			...
			var reply string
			err = client.Call("Greeter.SayHello", &Args{City: city}, &reply)
			if err != nil {
	...
	// Ruta para Swagger
	http.Handle("/apidocs/", httpSwagger.WrapHandler)
